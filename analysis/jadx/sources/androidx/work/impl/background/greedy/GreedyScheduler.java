...
public class GreedyScheduler implements Scheduler, OnConstraintsStateChangedListener, ExecutionListener {
    ...
    private final Map<WorkGenerationalId, InterfaceC0241b0> mConstrainedWorkSpecs = new HashMap();
    ...
    private void removeConstraintTrackingFor(WorkGenerationalId workGenerationalId) {
        InterfaceC0241b0 interfaceC0241b0Remove;
        synchronized (this.mLock) {
            interfaceC0241b0Remove = this.mConstrainedWorkSpecs.remove(workGenerationalId);
        }
    ...
    private long throttleIfNeeded(WorkSpec workSpec) {
        ...
        synchronized (this.mLock) {
            try {
                ...
                jMax = (((long) Math.max((workSpec.runAttemptCount - attemptData.mRunAttemptCount) - 5, 0)) * WorkRequest.DEFAULT_BACKOFF_DELAY_MILLIS) + attemptData.mTimeStamp;
            } catch (Throwable th) {
                throw th;
            }
        }
