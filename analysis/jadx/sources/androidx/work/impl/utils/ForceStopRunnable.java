            ...
            if (i >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    ...
                    for (int i7 = 0; i7 < historicalProcessExitReasons.size(); i7++) {
                        ApplicationExitInfo applicationExitInfoG = D0.g(historicalProcessExitReasons.get(i7));
                        if (applicationExitInfoG.getReason() == 10 && applicationExitInfoG.getTimestamp() >= lastForceStopEventMillis) {
                            return true;
                        }
                    }
                }
                ...
                while (true) {
                    try {
                        ...
                        try {
                            ...
                            this.mRetryCount = i;
                            if (i >= 3) {
                                String str = l6.d.x(this.mContext) ? "The file system on the device is in a bad state. WorkManager cannot access the app's internal data store." : "WorkManager can't be accessed from direct boot, because credential encrypted storage isn't accessible.\nDon't access or initialise WorkManager from directAware components. See https://developer.android.com/training/articles/direct-boot";
                                Logger logger = Logger.get();
                                String str2 = TAG;
                                logger.error(str2, str, e7);
                                IllegalStateException illegalStateException = new IllegalStateException(str, e7);
                                InterfaceC1752a initializationExceptionHandler = this.mWorkManager.getConfiguration().getInitializationExceptionHandler();
                                ...
                                Logger.get().debug(TAG, "Retrying after " + j, e7);
                                sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
                            }
                        }
                        long j7 = ((long) i) * BACKOFF_DURATION_MS;
                        Logger.get().debug(TAG, "Retrying after " + j7, e7);
                        sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
                        ...
                        InterfaceC1752a initializationExceptionHandler2 = this.mWorkManager.getConfiguration().getInitializationExceptionHandler();
