...
Production shell and PMU label/history asset checks return 200/pass. Signed-out and signed-in betslip layouts at 375, 768 and 1280 have no horizontal overflow; sampled console has no errors. All 31 current PDF races now have runner fields from the existing sync. Signed-in selection, minimum stake rejection, valid stake enablement, bet history, wallet and leaderboard were checked without submitting a test wager. Admin-session QA remains T059. Local verification passes 255 repository tests, 138 SQL assertions and the production build. Older ATR gate instructions below are historical.

The PMU worker now fills only completely empty, future canonical fields with a uniquely matched title/date/course/race/time and stores participant evidence. It preserves existing fields and accepted bets. No live backfill write was needed during this verification; database tests cover that mutation. Result-only shortened titles require complete stored runner-number/name agreement, checked again by SQL. The previously unmatched Craon R5 now validates with winner 9 JABRACADABRA and definitive dividend 2.90 per euro. PMU does not cover today's York meeting; those results cannot be promised automatic settlement.
