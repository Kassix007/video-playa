...
### Administrator leaderboard reset (2026-09-07)

Add a round number and reset timestamp to the singleton game settings. A protected idempotent administrator RPC locks settings and wallets, refuses to run while any bet is pending, records one audit event, restores each wallet to the configured starting balance through an `ADMIN_ADJUSTMENT` ledger entry, and advances the round. Recreate the public leaderboard so bet/win counts and net result use only the active round while balance continues to come from the reconciled wallet cache. Exclude the reset adjustments themselves from round profit/loss. The Admin UI requires a reason and the exact phrase `RESET LEADERBOARD`; it describes the preserved history and pending-bet gate before submission.
